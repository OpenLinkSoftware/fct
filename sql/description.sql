  ...
  if (fct_server_supports_invfp())
    {
      ...
      -- dbg_obj_princ ('b3s_render_invfp_opts () got invfp=', invfp, ' new server');
      http ('
  ...
  if (fct_server_supports_invfp())
    {
      ...
      -- dbg_obj_princ ('b3s_render_sas_opts () got sas=', sas, ' new server');
      http ('
  ...
  if (sid is not null)
    {
      for select top 1 fct_state from fct_state where fct_sid = sid do
        {
	  declare i varchar;
          -- dbg_obj_princ ('b3s_parse_inf () found fct_state=', fct_state);
          _inf := fct_inf_val (fct_state);
          _invfp := fct_invfp_val (fct_state);
          _sas := fct_sas_val (fct_state);
...
-- URL params override
  ...
  _inf		:= coalesce (get_keyword ('inf'		, params), _inf		);
  _invfp	:= coalesce (get_keyword ('invfp'	, params), _invfp	);
  _sas		:= coalesce (get_keyword ('sas'		, params), _sas		);

  if (not exists (select 1 from SYS_RDF_SCHEMA where rs_name = _inf))
    _inf := null;
  if (fct_server_supports_invfp())
    {
      if (not (_invfp in ('IFP', 'IFP_O', 'IFP_OFF', 'IFP_S')))
        {
          -- dbg_obj_princ ('ill _invfp=', _invfp);
          _invfp := null;
        }
      if (not (_sas in ('SAME_AS', 'SAME_AS_O', 'SAME_AS_OFF', 'SAME_AS_P', 'SAME_AS_S', 'SAME_AS_S_O')))
        {
          -- dbg_obj_princ ('ill _sas=', _sas);
          _sas := null;
        }
    }
  else
  ...
  -- dbg_obj_princ ('b3s_parse_inf() has set inf=', connection_get ('inf'), ', invfp=', connection_get ('invfp'), ', sas=', connection_get ('sas'), ', graphs=', connection_get ('graphs'));
}
;
