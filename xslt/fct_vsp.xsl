...
<xsl:template match="facets">
  ...
  if ($('pivot_a_ctr')) {
    ...
    pivotedit_a.href='/pivot_collections/pivot.vsp?sid=<xsl:value-of select="$sid"/>&amp;edit=1&amp;pagesize=0&amp;limit=500&amp;qrcodes=0&amp;CXML_redir_for_subjs=DESCRIBE&amp;CXML_redir_for_hrefs=DESCRIBE&amp;q=<xsl:value-of select="urlify (normalize-space(/facets/sparql))"/>'
	  ...
	  pivot_qrcode_opts.innerHTML = '&nbsp;&nbsp;<a href="#" title="Include a QRcode adjacent to each item\'s image" style="visibility:hidden" id="pivot_qrcode_opts_label">with QRcodes</a><input type="checkbox" onclick="fct_set_pivot_qrcode_opt()" id="pivot_qrcode" style="visibility:hidden"/>';
