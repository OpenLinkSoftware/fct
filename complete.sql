...
create procedure num_str (in n int)
{
  declare s varchar;
 s := '1234';
  s[0] := bit_shift (n, -24);
...
create procedure str_inc (in str varchar, in pref int := 0)
{
...
create procedure cmp_find_iri (in str varchar, in no_name int := 0)
{
  ...
  name[3] := id;
  iris :=  (select vector_agg (ri_name) from (select top 20 ri_name from rdf_iri where ri_name >= name and ri_name < str_inc (name, 4)) ir);
  if (length (iris) < 20 and length (iris) > 1)
    iris := (select vector_agg (ri_name) from (select ri_name from rdf_iri where ri_name >= name and ri_name < str_inc (name, 4) order by  iri_rank (ri_id) desc) ir);
...
create procedure cmp_find_ns (in str varchar)
{
  declare nss any;
  nss := (select vector_agg (rp_name) from (select top 20 rp_name from rdf_prefix where rp_name >= str and rp_name < str_inc (str)) ns);
  return nss;
}



create procedure cmp_with_ns (in str varchar)
{
 ...
 col := position (':', str);
  if (col = 0)
    return null;
  pref_str := (select ns_url from SYS_XML_PERSISTENT_NS_DECL where ns_prefix = subseq (str, 0, col - 1));
  if (pref_str is null)
    return null;
...
create procedure cmp_uri (in str varchar)
{
